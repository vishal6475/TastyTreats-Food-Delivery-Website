...
class TestCustomersController(BaseTestCase):
    ...
    def test_get_addresses_by_id(self):
        ...
        """
        response = self.client.open(
            '/v1/customers/{customer_id}/addresses'.format(customer_id='customer_id_example'),
            method='GET')
        self.assert200(response,
                       'Response body is : ' + response.data.decode('utf-8'))

    def test_get_cards_by_id(self):
        """Test case for get_cards_by_id

        Get all customer's cards by customer id
        """
        response = self.client.open(
            '/v1/customers/{customer_id}/cards'.format(customer_id='customer_id_example'),
            method='GET')
        self.assert200(response,
                       'Response body is : ' + response.data.decode('utf-8'))
